...
pub async fn launch_instance(
    ...
    let vanilla_json = fetch_version_json(
        client,
        data_dir,
        &instance.minecraft_version,
        &manifest_entry.url,
    ...
    let client_jar =
        download_client_jar(client, data_dir, &instance.minecraft_version, &version_json).await?;
...
async fn download_libraries(
    ...
    for lib in &libraries {
        ...
        let (url, path, sha1, _size) = if let Some(ref dl) = lib.downloads {
